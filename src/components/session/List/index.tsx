...
const CARD_BASE = "mx-4 mb-4 rounded-3xl overflow-hidden bg-background0 bg-surface shadow-soft";
        ...
        () => (
            <View className={`${CARD_BASE} p-5`}>
                <View className="flex-row gap-5 mb-5">
                    <SkeletonLoader width={112} height={140} style={{borderRadius: 24}}/>
                    <View className="flex-1 gap-3">
                        <SkeletonLoader width="85%" height={20} style={{borderRadius: 10}}/>
                        <SkeletonLoader width="55%" height={16} style={{borderRadius: 8}}/>
                        <SkeletonLoader width="40%" height={16} style={{borderRadius: 8}}/>
    ...
    const renderItem = useCallback(
        ({item}: { item: any }) => {
            ...
            const getStatusBadge = (status: string) => {
                ...
                if (status === "failed") {
                    return <View className={`px-3 py-1 rounded-full bg-error/10 border border-error/20`}>
                        <Text className="text-xs text-error tracking-wide">
                            处理失败
                        </Text>
                    </View>;
                }
                ...
                return <View className={`px-3 py-1 rounded-full bg-warning/10 border border-warning/20`}>
                    <Text className="text-xs text-warning tracking-wide">
                        处理中
                    </Text>
                </View>
            }
                    ...
                    <View className="p-3">
                        <View className="flex-row gap-5 mb-5">
                            ...
                            <View className="flex-1">
                                <Text className="text-foreground text-lg font-semibold leading-6 mb-1"
                                      numberOfLines={2}>
                                    {fallbackTitle}
                                </Text>
                                ...
                                <View className="flex-row flex-wrap gap-2">
                                    ...
                                    {script?.segments?.length ? (
                                        <View className="px-3 py-1 rounded-full bg-background2">
                                            <Text className="text-xs text-grey2">{script.segments.length} 段</Text>
                                        </View>
                                    ) : null}
                                </View>
                            </View>
                        ...
                        <View className="flex-row justify-between items-center">
                            ...
                            >
                                <Text
                                    className="text-xs font-semibold text-primary">查看详情</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                </Pressable>
                ...
                <FlatList
                    ...
                    ListEmptyComponent={() => (
                        <View className="flex-1 items-center justify-center py-32 px-6">
                            ...
                            <Text className="text-lg font-semibold text-foreground mb-1">暂无会话</Text>
                            <Text className="text-sm text-grey3 mb-6 text-center">创建第一个会话开始使用</Text>
                            <TouchableOpacity onPress={refetch} className="px-8 py-3 rounded-full bg-primary">
                                <Text className="text-white font-semibold text-sm">重试</Text>
                            </TouchableOpacity>
                        </View>
                    )}
                    ListFooterComponent={
