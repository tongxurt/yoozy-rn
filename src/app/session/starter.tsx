...
const Starter = () => {
  ...
  const [text, setText] = useState(""); // https://v.douyin.com/POVeTk_M8gU/

  // 计算是否禁用按钮
  const isDisabled = !text && !files.length;
  ...
  const confirm = async () => {
    ...
    const session = await createSession({ url: text, images });
  ...
  return (
    <View className={"flex-1"}>
      <View className={"px-5 pb-4 flex-row justify-between items-center"}>
        <Text className={"text-[22px] text-white font-bold"}>智能成片</Text>
