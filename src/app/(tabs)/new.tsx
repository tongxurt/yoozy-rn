...
// 功能项组件
function FeatureItem({item, onPress}: { item: typeof FEATURE_ITEMS[0], onPress: () => void }) {
    return (
        <View className={'px-5'}>
            <TouchableOpacity
                onPress={onPress}
            ...
            >
                <View className={'flex-row items-center'}>
                    ...
                    {item.sparkle && (
                        <Ionicons name="sparkles" size={14} color="#A855F7" style={{marginRight: 6}}/>
                    )}
    ...
    return (
        <View className={'flex-1 bg-background'}>
            ...
            <View className={'px-5 pb-6 pt-2'}>
                <View className={'bg-gradient-to-r from-purple-500/20 to-pink-500/20 rounded-2xl p-1'}>
                    <TouchableOpacity
                        onPress={handleCreate}
                        className={'bg-background0/80 rounded-xl p-4 flex-row items-center justify-between'}
                        activeOpacity={0.8}
                        style={{
                            shadowColor: '#A855F7',
